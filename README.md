...
- **リハ運用フィードバック反映**: 実戦リハーサルでの気づきを受けた改善。
  - 回答テキスト入力（任意機能）を廃止し、口頭発表が基本という運用方針を徹底。
  - 阻止する側の前進停止（指名）と投票開始を1アクションに統合。旧仕様は`NOMINATE`(前進停止のみ)→`ANSWER_DONE`(投票開始)の2イベントに分かれていたが、両者の間にワンクッション挟む必要はなく操作的には同時でよいと判断し、`ANSWER_DONE`イベントを廃止して`NOMINATE`が前進停止・マーカー凍結・投票開始（`audienceVotes`リセット・`votingRoundId`加算・`currentAnswerer`更新）を一括で行うようにした。
  ...
  - 各チームの「今の回答者」（`currentAnswerer`、現在アクティブな回答を書いたメンバー名）をサーバ権威で保持するようにし、`currentWriter`に加えて両チーム分をまとめて返す`currentWriters`をengineに追加。MC操作卓の状態パネルに常時表示するほか、投影画面では各チームのサイドに「次の回答者」欄を常設して表示する。
  - MC操作卓・投影画面共通のマーカーバー下のステータス表示を、`🔴進行中`/`🔵進行中`/`両チーム回答中`/`投票中`/`準備中`/`試合終了`という短いバッジ表記に統一し、回答者側が今すべきことを一言で示す指示文（例:「🔴 回答ができたら指名（挙手）してください」）を追加。投影画面の見出しもこの共通ロジックを再利用するようにした。
